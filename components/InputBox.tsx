...
export const InputBox: React.FC<InputBoxProps> = ({ onSend, onEnterLiveMode, onOpenIntegrations }) => {
  ...
  const handleSend = () => {
    ...
    if (!trimmed) return;
    onSend(trimmed, isAgentMode, { skill, webSearch: isWebSearch });
    setValue("");
    setSkill("none");
  };
  ...
  return (
    <div className={`relative rounded-[20px] bg-[color:var(--vynthen-bg-secondary)] transition-all duration-200 ${isAgentMode
      ...
      {/* Bottom bar */}
      <div className="flex items-center justify-between px-3 pb-3 border-t border-transparent">
        {/* Left: 3-dot menu */}
        <div className="relative" ref={menuRef}>
          {isMenuOpen && (
            <>
              ...
              <div className="absolute bottom-10 left-0 z-50 w-52 rounded-2xl bg-[color:var(--vynthen-bg)] border border-[color:var(--vynthen-border)] shadow-xl p-1.5">
                ...
                <p className="px-3 py-1.5 text-[10px] font-semibold text-[color:var(--vynthen-fg-muted)] uppercase tracking-widest">Tools</p>

                {/* Web Surf toggle */}
                <button type="button" onClick={() => { setIsWebSearch(v => !v); setIsMenuOpen(false); }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left text-[13px] font-medium transition-colors ${isWebSearch ? "bg-[color:var(--vynthen-bg-secondary)] text-[color:var(--vynthen-fg)] border border-[color:var(--vynthen-border)]" : "text-[color:var(--vynthen-fg-muted)] hover:bg-[color:var(--vynthen-bg-secondary)] hover:text-[color:var(--vynthen-fg)]"}`}>
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${isWebSearch ? "bg-blue-500/20 text-blue-400" : "bg-transparent"}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
                      <circle cx="12" cy="12" r="10" />
                      <path strokeLinecap="round" strokeLinejoin="round" d="M2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" />
                    </svg>
                  </div>
                  <span className={isWebSearch ? "font-semibold" : ""}>Web Surf</span>
                  {isWebSearch && <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5 ml-auto text-[color:var(--vynthen-fg-muted)]"><path fillRule="evenodd" d="M19.916 4.626a.75.75 0 01.208 1.04l-9 13.5a.75.75 0 01-1.154.114l-6-6a.75.75 0 011.06-1.06l5.353 5.353 8.493-12.739a.75.75 0 011.04-.208z" clipRule="evenodd" /></svg>}
                </button>
          ...
          <button type="button" onClick={() => setIsMenuOpen(v => !v)} aria-label="More options"
            className={`flex h-8 w-8 items-center justify-center rounded-full transition-all ${isAgentMode ? "bg-bw-rainbow text-[color:var(--vynthen-bg)] shadow-sm" :
              isWebSearch ? "bg-blue-500/20 text-blue-400" :
